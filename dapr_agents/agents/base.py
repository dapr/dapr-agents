...
class AgentBase(AgentComponents):
    ...
    def _construct_messages_with_instance_history(
        ...
        logger.debug(
            f"_construct_messages: instance has {len(instance_messages)} non-system messages"
        )
        ...
        logger.debug(
            f"_construct_messages: persistent memory has {len(persistent_memory)} messages"
        )
        ...
        def _make_message_key(msg: Dict[str, Any]) -> tuple:
            ...
            if tool_calls:
                tool_call_ids = tuple(
                    sorted(tc.get("id") for tc in tool_calls if tc.get("id"))
                )
    ...
    def _process_user_message(
        ...
        logger.debug(
            f"_process_user_message: Added user message to memory. Total messages in memory: {len(self.memory.get_messages())}"
        )
        self.save_state()
    ...
    def _save_assistant_message(
        ...
        logger.debug(
            f"_save_assistant_message: Added assistant message to memory. Total messages in memory: {len(self.memory.get_messages())}"
        )
        self.save_state()
