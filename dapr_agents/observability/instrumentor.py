...
class DaprAgentsInstrumentor(BaseInstrumentor):
    ...
    def _apply_context_propagation_fix(self) -> None:
        ...
        try:
            ...
            logger.info("✅ Successfully imported Dapr Workflow modules for context propagation")
            ...
            def run_sync_with_context(coro):
                ...
                if context_api:
                    ...
                    logger.debug(f"🔗 Captured context for async execution: {current_span.get_span_context() if current_span else 'No span'}")
            ...
            def make_context_aware_task_wrapper(self, task_name: str, method, task_instance):
                """
                ...
                def wrapper(ctx, *args, **kwargs):
                    ...
                    from .context_storage import store_workflow_context, get_workflow_context
                    
                    if instance_id:
                        # Retrieve global W3C context and associate with instance
                        global_context = get_workflow_context('__global_workflow_context__')
                        if global_context and global_context.get('traceparent'):
                            logger.debug(f"🔗 Storing W3C context for instance {instance_id}")
                            logger.debug(f"🔗 Traceparent: {global_context.get('traceparent')}")
                            
                            # Store W3C context with specific workflow instance ID
                            store_workflow_context(instance_id, global_context)
                        else:
                            logger.warning("⚠️ No global workflow context found for storage")
                    else:
                        logger.warning("⚠️ No workflow instance_id - cannot store W3C context")
                    ...
                    try:
                        ...
                        logger.debug(f"✅ Completed context-aware workflow task: {task_name}")
                        return result
                    except Exception as e:
                        logger.exception(f"❌ Context-aware workflow task '{task_name}' failed: {e}")
                        raise
                
                return wrapper
            
            # Apply enhanced context-aware task wrapper to Dapr Workflow runtime
            logger.debug("🔧 Monkey patching WorkflowApp._make_task_wrapper for W3C context propagation")
            ...
            logger.debug("✅ Applied W3C context propagation enhancement for Dapr Workflow tasks")
            
        except ImportError as e:
            logger.warning(f"Could not apply W3C context propagation fix (ImportError): {e}")
        except Exception as e:
            logger.error(f"Error applying W3C context propagation fix: {e}", exc_info=True)
    ...
    def _apply_llm_wrappers(self) -> None:
        ...
        try:
            ...
            for chat_client_class, module_name in chat_client_classes:
                ...
                logger.debug(f"Instrumented {chat_client_class.__name__} in {module_name}")
            ...
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if (isinstance(attr, type) and 
                        ...
                        chat_client_classes.append((attr, f'dapr_agents.llm.{provider}.chat'))
