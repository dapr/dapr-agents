...
def convert_single_message_to_openinference(message: Any, index: int) -> Optional[Dict[str, Any]]:
    """
    ...
    if isinstance(message, dict):
        ...
        if tool_calls and role == "assistant":
            ...
            if oi_tool_calls:
                ...
                logger.debug(f"Converted {len(oi_tool_calls)} tool calls for message {index}")
...
def get_input_message_attributes(oi_messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if OPENINFERENCE_AVAILABLE:
        try:
            ...
            logger.debug(f"Generated {len(attributes)} input message attributes using OpenInference")
            return attributes
        except Exception as e:
            logger.warning(f"OpenInference get_llm_input_message_attributes failed: {e}, using fallback")
            return {}
    else:
    ...
    for i, tool in enumerate(tools):
        try:
            ...
            if hasattr(tool, 'to_function_call'):
                ...
                logger.debug(f"Extracted schema for tool {i}: {function_call.get('name', 'unknown')}")
                
            elif hasattr(tool, 'name'):
                # Fallback: create basic schema from tool attributes
                tool_schema = create_fallback_tool_schema(tool)
                attributes[f"llm.tools.{i}.tool.json_schema"] = safe_json_dumps(tool_schema)
...
def get_output_message_attributes(oi_messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if OPENINFERENCE_AVAILABLE:
        try:
            ...
            logger.debug(f"Generated {len(attributes)} output message attributes using OpenInference")
            return attributes
        except Exception as e:
            logger.warning(f"OpenInference get_llm_output_message_attributes failed: {e}, using fallback")
...
def get_fallback_output_attributes(oi_message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    attributes = {}
    if oi_message:
        attributes[f"{LLM_OUTPUT_MESSAGES}.0.{MESSAGE_ROLE}"] = oi_message.get("role", "assistant")
        attributes[f"{LLM_OUTPUT_MESSAGES}.0.{MESSAGE_CONTENT}"] = oi_message.get("content", "")
