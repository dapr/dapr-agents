...
try:
    ...
    raise ImportError("OpenInference not installed - please install with `pip install dapr-agents[observability]`")
...
class WorkflowRunWrapper:
    ...
    def __call__(self, wrapped: Any, instance: Any, args: Any, kwargs: Any) -> Any:
        ...
        workflow_name = workflow if isinstance(workflow, str) else getattr(workflow, '__name__', 'unknown_workflow')
        ...
        logger.debug(f"🔍 WorkflowRunWrapper creating AGENT span for workflow: {workflow_name}")
        ...
        with self._tracer.start_as_current_span(span_name, attributes=attributes) as span:
            ...
            try:
                ...
                span.set_attribute(OUTPUT_VALUE, safe_json_dumps({"instance_id": instance_id}))
...
class WorkflowMonitorWrapper:
    ...
    def __call__(self, wrapped: Any, instance: Any, args: Any, kwargs: Any) -> Any:
        ...
        # Debug logging to confirm wrapper is being called
        logger.debug(f"🔍 WorkflowMonitorWrapper creating AGENT span for workflow: {workflow_name}")
        
        # Handle async execution
        return self._handle_async_execution(wrapped, args, kwargs, attributes, workflow_name)
    
    def _build_monitor_attributes(self, instance: Any, workflow_name: str, 
                                arguments: Dict[str, Any]) -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    try:
                        ...
                        if captured_context.get('traceparent'):
                            logger.debug(f"🔗 Captured workflow context in AGENT span")
                            logger.debug(f"🔗 Traceparent: {captured_context.get('traceparent')}")
                            
                            # Store context IMMEDIATELY for workflow tasks to use
                            store_workflow_context('__global_workflow_context__', captured_context)
                            logger.debug("🔗 Stored global workflow context for task correlation")
                        else:
                            logger.warning("⚠️ No traceparent found in captured context")
                    except Exception as e:
                        logger.warning(f"⚠️ Failed to capture/store workflow context: {e}")
