...
class WorkflowTaskWrapper:
    ...
    def __call__(self, wrapped: Any, instance: Any, args: Any, kwargs: Any) -> Any:
        ...
        if context_api and context_api.get_value(context_api._SUPPRESS_INSTRUMENTATION_KEY):
        ...
        task_name = getattr(instance.func, '__name__', 'unknown_task') if instance.func else 'workflow_task'
        span_kind = self._determine_span_kind(instance, task_name)
        
        # Build span attributes
        attributes = self._build_task_attributes(instance, ctx, payload, span_kind, task_name)
        ...
        if asyncio.iscoroutinefunction(wrapped):
            return self._handle_async_execution(wrapped, args, kwargs, span_name, attributes)
        else:
            return self._handle_sync_execution(wrapped, args, kwargs, span_name, attributes)
    ...
    def _determine_span_kind(self, instance: Any, task_name: str) -> str:
        ...
        if (hasattr(instance, 'llm') and instance.llm is not None) or 'generate_response' in task_name:
            return LLM
        
        # Check if this is a tool execution task  
        if (hasattr(instance, 'agent') and instance.agent is not None) or 'run_tool' in task_name:
            return TOOL
    ...
    def _build_task_attributes(self, instance: Any, ctx: Any, payload: Any, span_kind: str, task_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if ctx:
            ...
            attributes["workflow.correlation_id"] = instance_id  # Additional correlation
            
            # Add resource-level attributes for Phoenix UI grouping
            attributes["resource.workflow.instance_id"] = instance_id
            attributes["resource.workflow.name"] = "ToolCallingWorkflow"  # Could be dynamic
            ...
            if current_span:
                ...
                logger.debug(f"⚡️ Task in instance {instance_id} starting with trace_id=0x{current_trace_id:x} (will be restored via W3C context)")
        ...
        if span_kind == TASK and task_name:
            ...
            attributes["workflow.task.category"] = self._categorize_workflow_task(task_name)
    ...
    def _handle_async_execution(self, wrapped: Any, args: Any, kwargs: Any,
                              span_name: str, attributes: dict) -> Any:
        """
        ...
        async def async_wrapper():
            ...
            with create_child_span_with_context(self._tracer, span_name, otel_context, attributes) as span:
                ...
                if otel_context:
                    logger.debug(f"🔗 Creating {task_category} span with RESTORED context: {span_name}")
                    logger.debug(f"👨‍👦 Restored from traceparent: {otel_context.get('traceparent', 'None')}")
                else:
                    logger.debug(f"⚠️ Creating {task_category} span WITHOUT context restoration: {span_name}")
                
                logger.debug(f"🎯 Current span context: {current_span.get_span_context() if current_span else 'None'}")
    ...
    def _handle_sync_execution(self, wrapped: Any, args: Any, kwargs: Any,
                             span_name: str, attributes: dict) -> Any:
        """
