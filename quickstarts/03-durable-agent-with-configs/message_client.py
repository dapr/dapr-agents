...
BLOG_TOPIC = os.getenv("BLOG_TOPIC", "Weather Information")  # used when RAW_DATA is not provided
...
def _build_payload() -> Dict[str, Any]:
    ...
    if RAW_DATA:
        ...
        payload = {
            "task": "What is the weather like in San Francisco and Seattle?",
        }

    # Derive task from other fields if still missing
    if "task" not in payload:
        topic = payload.get("topic") or BLOG_TOPIC
        payload["task"] = f"What is the weather like in {topic}?"
