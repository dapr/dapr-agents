...
build_images () {
  ...
  docker push localhost:5001/github-agent:latest 
  echo "#### Images pushed! ####"
}
...
# We fetch the compents from the k8s workflow to not duplicate them
kubectl apply -f "${BASE_DIR}/../06-k8s-multi-agent-workflow/components/" &>/dev/null
